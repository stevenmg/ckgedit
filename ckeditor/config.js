...
'FFFFFF','FFFFFF','FFFFFF'
	...
	// http://docs.ckeditor.com/#!/api/CKEDITOR.config
      top.ckgedit_language_chk(config);  //resets language if $conf['other_lang'] != 'default' and resets auto scayt to true of false
     // config.scayt_autoStartup = true;
      config.format_tags = 'p;h1;h2;h3;h4;h5';
      config.extraSpecialChars  = ['↔'];
      config.extraPlugins = 'signature,footnote,shortcuts,fontAssist,tags,timestamp,headerbuttons'; //,devtools';
     ...
     'Signature':'Signature' ,'Footnotes':'Footnotes', 'Tags':""
     };
   	if(top.hasTags()){
		config.ToolBarItemsAlt['Tags']='Tags';
	}

     var xtra = top.extra_plugins();
	...
	{name: 'header_buttons', items: ['same_header', 'lower_header', 'higher_header', 'none_header']},
		...
		{ name: 'insert', items : [ 'Image','Table','HorizontalRule','Smiley','SpecialChar',  config.ToolBarItemsAlt['Signature'] ,config.ToolBarItemsAlt['Footnotes'],config.ToolBarItemsAlt['Tags'] ]},
